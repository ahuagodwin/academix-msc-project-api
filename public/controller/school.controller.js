...
const createSchool = async (req, res) => {
    ...
    try {
        ...
        const existingSchools = await school_model_1.default.find({
        ...
        const uniqueNewSchools = newSchoolNames.filter((name) => !existingSchoolNames.includes(name));
        ...
        const newSchools = uniqueNewSchools.map((name) => {
            ...
            return {
                _id,
                name,
                code: (0, Helpers_1.generateInitials)(name),
                faculties: [],
                schoolId: _id,
            };
        });
        await school_model_1.default.insertMany(newSchools, { session });
        // Commit transaction
...
const updateSchool = async (req, res) => {
    ...
    try {
        ...
        const existingSchool = await school_model_1.default.findOne({ name: new RegExp(`^${normalizedNewName}$`, "i") }).session(session);
        ...
        school.code = (0, Helpers_1.generateInitials)(normalizedNewName);
...
const deleteSchool = async (req, res) => {
    ...
    try {
        ...
        // Delete the school
        await school_model_1.default.deleteOne({ _id: schoolId }).session(session);
        // Commit transaction
        await session.commitTransaction();
        res.status(200).json({
...
const getSchoolById = async (req, res) => {
    try {
        ...
        const school = await school_model_1.default.findById(schoolId).select("-__v -_id");
        if (!school) {
            res.status(404).json({ error: "School not found", status: false });
            return;
        }
...
const createFaculty = async (req, res) => {
    ...
    try {
        ...
        const existingFacultyNames = new Set(school.faculties.map((faculty) => faculty.name.toLowerCase().trim()));
        // Filter out duplicates
        const uniqueNewFaculties = newFaculties.filter(faculty => !existingFacultyNames.has(faculty.toLowerCase()));
...
const updateFaculty = async (req, res) => {
    ...
    try {
        ...
        const facultyExists = school.faculties.some((fac) => fac.name.toLowerCase() === name.toLowerCase() && fac._id.toString() !== facultyId);
        if (facultyExists) {
            await session.abortTransaction();
...
const getAllFaculties = async (req, res) => {
    try {
        ...
        // Find the school
        const school = await school_model_1.default.findById(schoolId).select("faculties");
        if (!school) {
            res.status(404).json({ error: "School not found", status: false });
            return;
        }
...
const createDepartments = async (req, res) => {
    ...
    try {
        ...
        // Find the faculty within the school
        const faculty = school.faculties.find((faculty) => new RegExp(`^${faculty?.facultyId}$`, "i").test(facultyId));
        if (!faculty) {
            await session.abortTransaction();
        ...
        const existingDepartmentNames = new Set(faculty.departments.map((dept) => dept.name.toLowerCase()));
        // Filter out duplicates
        const uniqueNewDepartments = newDepartments.filter((dept) => !existingDepartmentNames.has(dept.toLowerCase()));
...
const updateDepartment = async (req, res) => {
    ...
    try {
        ...
        const departmentExists = faculty.departments.some((dept) => dept.name.toLowerCase() === name.trim().toLowerCase());
        if (departmentExists) {
            await session.abortTransaction();
...
const deleteDepartment = async (req, res) => {
    ...
    try {
        ...
        // Find the department within the faculty
        const departmentIndex = faculty.departments.findIndex((dept) => dept._id.toString() === departmentId);
...
const getDepartmentById = async (req, res) => {
    try {
        ...
        // Find the department within the faculty
        const department = faculty.departments.find((dept) => new RegExp(`^${dept?.departmentId}$`, "i").test(departmentId));
        if (!department) {
            res.status(404).json({ error: "Department not found", status: false });
            return;
        }
...
const createCourses = async (req, res) => {
    ...
    try {
        ...
        // Find the department within the faculty
        const department = faculty.departments.find((dept) => new RegExp(`^${dept?._id}$`, "i").test(departmentId));
        if (!department) {
            await session.abortTransaction();
        ...
        const existingCourseNames = new Set(department.courses.map((course) => course.name.toLowerCase()));
        // Filter out duplicates
        const uniqueNewCourses = newCourses.filter((course) => !existingCourseNames.has(course.toLowerCase()));
...
const updateCourse = async (req, res) => {
    ...
    try {
        ...
        const course = department.courses.find((c) => new RegExp(`^${c?._id}$`, "i").test(courseId));
        if (!course) {
            await session.abortTransaction();
        ...
        const existingCourseNames = department.courses.map((c) => c.name.toLowerCase());
...
const deleteCourse = async (req, res) => {
    ...
    try {
        ...
        const courseIndex = department.courses.findIndex((c) => new RegExp(`^${c?._id}$`, "i").test(courseId));
        if (courseIndex === -1) {
            await session.abortTransaction();
            res.status(404).json({ error: "Course not found", status: false });
            return;
...
const getCourseById = async (req, res) => {
    try {
        ...
        // Find the course within the department
        const course = department.courses.find((course) => new RegExp(`^${course?._id}$`, "i").test(courseId));
        if (!course) {
