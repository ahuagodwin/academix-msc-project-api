...
exports.requestFilePermissions = exports.getSharedFilesByGroup = exports.getSharedFiles = exports.deleteShareFile = exports.updateShareFile = exports.shareFile = void 0;
...
const updateShareFile = async (req, res) => {
    try {
        const { shareId } = req.params;
        const { recipients, groupId, permissions } = req.body;
        const senderId = req.user?._id;
        // Find the shared file entry
        const sharedFile = await fileShare_model_1.default.findById(shareId);
        if (!sharedFile) {
        ...
        if (groupId !== undefined)
            sharedFile.groupId = groupId;
        if (permissions) {
            if (!Array.isArray(permissions) || !permissions.every(p => [types_1.UserPermission.READ, types_1.UserPermission.UPDATE, types_1.UserPermission.DOWNLOAD].includes(p))) {
...
exports.updateShareFile = updateShareFile;
const deleteShareFile = async (req, res) => {
    try {
        const { shareId } = req.params;
        const senderId = req.user?._id;
        // Find the shared file entry
        ...
        await fileShare_model_1.default.findByIdAndDelete(shareId);
...
const getSharedFilesByGroup = async (req, res) => {
    try {
        ...
        const sharedFiles = await fileShare_model_1.default.find({ groupId })
            .populate("file") // Populate file details
...
exports.getSharedFilesByGroup = getSharedFilesByGroup;
/**
 * Controller for requesting permissions on a shared file
 */
const requestFilePermissions = async (req, res) => {
    try {
        ...
        const fileShare = await fileShare_model_1.default.findOne({ file: fileId, recipients: recipientId });
        if (!fileShare) {
        ...
        const newPermissions = requestedPermissions.filter((perm) => !existingPermissions.includes(perm));
        if (newPermissions.length === 0) {
            res.status(400).json({ success: false, message: "You already have these permissions." });
            return;
        }
        // Send notification to the file owner
        const owner = await user_model_1.User.findById(file.userId);
        if (owner) {
            await (0, notification_1.sendNotification)({
                userId: owner._id,
